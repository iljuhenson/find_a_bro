...
def verify_token(token: str, db_session) -> UserModel | None:
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        # print(db_session)
        user_db = db_session.query(UserModel).filter(UserModel.email == payload['email'] and UserModel.id == payload['id']).first()
    except:
        user_db = None
